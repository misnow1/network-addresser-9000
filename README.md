...
- **The named volume is persistence, not a backup.** `docker compose down`/`up` preserves data, but there's no automated backup — take one with `docker compose exec db sh -c 'mariadb-dump -u root -p"$MARIADB_ROOT_PASSWORD" --all-databases' > backup.sql` (restore via `docker compose exec -T db sh -c 'mariadb -u root -p"$MARIADB_ROOT_PASSWORD"' < backup.sql`). The single-quoted inner command matters: it defers `$MARIADB_ROOT_PASSWORD` expansion to the container's own shell, where that variable is actually set — expanding it on the host instead yields an empty value and an interactive password prompt.
