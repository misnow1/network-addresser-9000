...
A Network Switch/Device Type is a **purpose profile** built on a hardware model, not the bare hardware model itself: the same physical switch or device commonly needs several different profiles, because what each port is *used for* varies even though the hardware doesn't. For example, a Cisco SG350-10MP wired up for a drive rack (different VLAN per port) needs a different profile than the identical switch wired for an amp rack; a Martin Audio IK-42 with a Dante card needs a different profile (extra ports) than the same model without one. `Name` is a **required, non-blank** profile label (e.g. "For Drive Rack", "with Dante Card", "Default" for a model with only one profile) — this keeps the type selector unambiguous for an audience that may not have deep VLAN/subnetting knowledge. A Network Switch Type is identified by `(Manufacturer, Model, Name)`. A Network Device Type is identified by `(Device Model, Name)` instead ([ADR 0026](./docs/adr/0026-device-model-entity.md)): the device side's manufacturer/model live on a separate Network Device Model entity that profiles point at (see below), so the two Type models deliberately diverge until the switch side is aligned the same way (issue #78).
...
* Network Device Model ([ADR 0026](./docs/adr/0026-device-model-entity.md)) — the bare hardware identity a Network Device Type profile is built on, e.g. "an Amphenol RJD32A3-0050", independent of what any particular profile's ports are wired for.
    * Manufacturer
    * Model
...
* Network Device Type Port (a port definition for the port(s) that an instance of this device will always have)
    ...
    * Slot Offset (`PositiveIntegerField`, default 0) — every static address in the system is `range_base + rack_slot + slot_offset` (ADR 0027), so this field is the whole addressing mechanism, not an exception carved out of an otherwise-editable address. A non-zero offset covers two hardware shapes: firmware that derives one port's address from another port's and refuses to let anyone change it (e.g. a DiGiCo console's audio engine, always its control address + 1), and a second, independently addressed interface on a VLAN the device already uses (e.g. a Yamaha console's Device Control interface — ADR 0027 decision 3, closing #42). A VLAN carrying a non-zero-offset port must also carry an offset-0 port on that same VLAN — the offset-0 port is what the offset is measured from. See [ADR 0017](./docs/adr/0017-derived-same-vlan-addresses.md) for the mechanism's origin and [ADR 0027](./docs/adr/0027-the-ordinal-is-the-unit.md) for its generalization; this is a narrow mechanism, not a general multi-part-hardware feature — see ADR 0017's scope-boundary section.
* Network Device (an instance of a Network Device Type). Network Switch and Network Device are separate hierarchies, not a shared type — see `CONTEXT.md`. An unracked Network Device/Switch (`rack` is null) is in the **Spare Pool**: DHCP-configured, tracked by little more than serial number and hostname until it's racked. A device claims the **set** of ordinals `{rack_slot + offset}` for each offset its Type declares (`claimed_offsets`, computed from the Type, never stored), not a contiguous range — a device with offsets `{0, 64}` claims exactly two ordinals and is indifferent to the 63 between them (ADR 0027 decision 2, closing #83); every other device still occupies exactly its own `rack_slot`, since that set is `{0}` whenever every port is at offset 0. `slot_span` (`max(Slot Offset) + 1`) still exists alongside `claimed_offsets`, kept only for the rack's `slot_count` bound, which genuinely is about the highest ordinal reached rather than the full claimed set.
    * ...
    * Serial Number
...
* Network Device Port: description/purpose/VLAN/Port Type/Slot Offset automatically populated (one-time copy, as with Network Switch Port above) when the device is created, from its Type's Network Device Type Ports. **Description, VLAN, physical Port Type, and Slot Offset are immutable.** The DHCP setting (and which switch port it's connected to) are editable; the **IP address is not**, for any static port, at any Slot Offset (ADR 0027 decision 1, superseding ADR 0003): every static address is derived — `range_base + rack_slot + slot_offset` — and system-written only, so the operator never types one. Shown in the admin, not hidden, with help text explaining that the rack slot sets it. **Flipping an existing DHCP port to static still works** and is the second edge that writes an address: the operator flips the toggle and the address is derived, not asked for (`_derive_address_on_flip_to_static()`) — ADR 0013's conversion, kept by ADR 0027 decision 1 rather than lost to it. It is refused while the device is unracked, where there is no rack slot to derive from. **Flipping an existing static port to DHCP also still works**, and is its own symmetric edge: the address is simply cleared, not re-derived — there is no formula to run in that direction, only the value to let go of. Device creation offers a DHCP-or-static choice, defaulting to static: a racked device's ports get a computed rack-range-base + rack-slot (+ Slot Offset) address per VLAN, the same way a Network Switch's address is suggested, all in one whole-device pass (`NetworkDevice._derive_addresses()`) rather than one port recomputing from another; an unracked device (spare pool) always materializes DHCP regardless of the choice, and a port on an L2-only VLAN always materializes DHCP too (see ADR 0013). A port's identity is `(Device, Description)` — Port Number, when present at all, is neither required nor unique.
...
* Within a rack's range, a device port's static address is the rack's base address plus the device's rack slot number, plus the port's Slot Offset (0 for every port except one carrying a declared offset — see Slot Offset above). This is stored per port, not recomputed on the fly, but it is **not editable, for any port, at any offset** — offset 0 included (ADR 0027 decision 1, superseding ADR 0003's original "stored but overridable" design). The operator never types an address now — the system derives it, at device creation or when an existing DHCP port is flipped to static; the device-replacement workflow ADR 0003 kept mutability open for (swapping a spare into an already-addressed slot) is served better by derivation — put the replacement in the same slot and the address follows. See [ADR 0003](./docs/adr/0003-device-addresses-stored-not-immutable.md) for the original design and [ADR 0027](./docs/adr/0027-the-ordinal-is-the-unit.md) for why it's superseded. See [ADR 0017](./docs/adr/0017-derived-same-vlan-addresses.md) for the non-zero-offset derivation mechanism this generalizes from.
...
An example generated hostname might be `mps-{rack}-{label}-{slot}` (using the restricted placeholder set recommended below, not a real templating engine). A generated name would be computed once at materialization time and stored, the same "computed once, not left to drift silently" instinct static addresses now follow (ADR 0027) — though addresses recompute on a rack-slot move (behind a confirmation) and a hostname built this way would need its own decision about whether to follow suit.
