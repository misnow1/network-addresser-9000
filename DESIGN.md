...
* Network Device Type (amp, processor, mixer, etc.) — also a purpose profile; see above. e.g. "Martin Audio IK-42 — with Dante Card" vs "— without Dante Card", or "Shure ULXD4Q — Split Mode" vs "— Redundant Mode".
    ...
    * Companion Type (`ForeignKey` to Network Device Type, self-referential, optional, `on_delete=PROTECT`) — for hardware that is genuinely two separate, mandatorily-paired rows: a Yamaha DM7C/DM3 console reaches stage boxes through a second Dante interface the production data already carries as its own device. A Type declaring a Companion Type requires exactly one companion device per instance and locks like every other identity field once the Type has instances (ADR 0010); a Type that is itself some other Type's Companion Type can never be instantiated standalone, and chains/self-reference are refused. This is existence and lifecycle only — **not** addressing: unlike Slot Offset, a companion's address is never derived, offset, or computed from its host's; see "Why not Slot Offset" in [ADR 0018](./docs/adr/0018-device-companions.md).
...
* Network Device (an instance of a Network Device Type). Network Switch and Network Device are separate hierarchies, not a shared type — see `CONTEXT.md`. An unracked Network Device/Switch (`rack` is null) is in the **Spare Pool**: DHCP-configured, tracked by little more than serial number and hostname until it's racked. A device whose Type declares a non-zero Slot Offset occupies an **ordinal range**, not a single slot: `rack_slot` through `rack_slot + max(Slot Offset)` across its Type's ports (`slot_span`, computed from the Type, never stored) — every other device still occupies exactly its own `rack_slot`, since `slot_span` is 1 whenever every port is at offset 0.
    ...
    * Host (`OneToOneField` to Network Device, self-referential, optional, `on_delete=CASCADE`) — set only on a companion device (ADR 0018), never editable directly: a host materializes its companion (and the companion's own ports) in the same transaction as itself, at creation, from its Type's Companion Type. The pair moves as a unit — the companion's Rack/Rack Slot are host-managed and locked at the model layer, following the host wherever it moves — but a companion is never independently creatable, re-parentable, or deletable: removing the host cascades to the companion, and removing the companion alone is refused with an error naming the host. The companion's own addresses are entirely its own, addressed exactly like any other device's ports from its own Type.
