    ...
    def handle(self, *args: Any, **options: Any) -> None:
        with transaction.atomic():
            ...
            if not vlan_created and (vlan.name != DEFAULT_VLAN_NAME or vlan.subnet != ""):
                self.stderr.write(
                    self.style.WARNING(
                        f"VLAN id={DEFAULT_VLAN_ID} already exists as {vlan.name!r} "
                        f"(subnet={vlan.subnet!r}) — leaving it as-is rather than overwriting."
                    )
                )
            ...
            elif profile.name != DEFAULT_PROFILE_NAME:
                self.stdout.write(
