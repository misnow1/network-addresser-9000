...
is touched, and `DESIGN.md` needs no amendment.
...
**A test factory already exists.** `tests.py:148–149` and `test_ui.py:98–99` are the
`_make_device_type` helper's `manufacturer`/`model` defaults. Most of the 96 lines flow through it.
Extend the helper to create-or-get a `NetworkDeviceModel` and the great majority of call sites need
no edit at all. Do this **first** and re-count before touching anything by hand.
...
1. `RunPython` — collapse and re-point (below).
2. `AlterField` `device_model` → `null=False`.
3. **`RemoveConstraint("unique_device_type")`** — must come *before* step 5.
4. `AlterModelOptions` — `ordering` off `manufacturer`/`model`.
5. `RemoveField` `manufacturer`, `RemoveField` `model`.
6. `AddConstraint` — `unique_device_type` on `["device_model", "name"]`.

Steps 3 and 4 are the footgun: the old constraint and the old `Meta.ordering` both name columns that
step 5 drops, and a `RemoveField` under a live constraint fails on MariaDB. `0004_…` already
sequences a `unique_device_type` reshape this way (`:81` remove, `:136` re-add) — follow it.

Split into two files because a `RunPython` cannot reliably see a column added in the same migration,
and because it makes the data step independently reversible. Both ship in PR 1; the split is
internal.
...
- `2076` `list_display` and `2078` `search_fields`: `"manufacturer"`, `"model"` → `"device_model"`.
...
- **The lock**: with instances, re-pointing `device_type.device_model` raises from both `save()` and
  `clean()`; **without** instances, it succeeds.
...
- **The migration.** The suite already has a migration-test harness — `tests.py:6584`, `6618`,
  `6634–6635`, `6651–6652` use `apps.get_model("inventory", "NetworkDeviceType")`. Follow it: 3
  types across 2 models collapse to 2 rows with FKs correctly re-pointed.
...
- **The test diff could still be large** if `_make_device_type` does not cover as many of the 96
  sites as the census suggests. Extend the helper and re-count *before* committing to a hand sweep.
