...
**Owner**:
Who owns a piece of equipment — "MPS", "BEJ" — a table rather than free text for the same reason as Department (issue #10). It is the first, **blocking** component of a computed hostname (ADR 0023): no owner means no computed name at all, unlike the optional components. `Rack.owner` is a creation-time *default* for a racked item's own `owner`, not inheritance — moving a rack to a different owner never touches equipment already in it (ADR 0019's suggest-don't-lock), and the importer seeds only `Rack.owner`, never a per-device backfill. See ADR 0023.
_Avoid_: confusing this with Department (ADR 0021) — Department owns a VLAN and is purely descriptive vocabulary nothing branches on; Owner owns equipment and blocks hostname computation when absent; reading `Rack.owner` as inheritance rather than a creation-time default; confusing this with the VLAN map's "Owner" column (`vlan_map.html`) — that heading predates this model and means *the equipment holding this address*, not who owns it; it is not being renamed

**Rack**:
An abstract grouping of equipment with a fixed slot count and a reserved IPv4 address range per VLAN, used to compute static addresses for the equipment installed in it. A slot is an *addressing ordinal* (base address + slot number) — not a physical rack-unit position; physical RU height/placement is deliberately not modeled. **A Rack need not correspond to physical hardware at all**: `AVIO`, `CONSOLES` and `SHURE` are groupings of related equipment rather than enclosures, and they address exactly as an amp rack does. Rack is this system's address pool, and the only thing that allocates addresses — see ADR 0019, which declined a separate pool concept and records why the ordinal is what guarantees a device's addresses stay consistent across VLANs. The ordinal is *suggested* (the lowest rack slot at which every ordinal the occupant would claim is free — ADR 0027's `claimed_offsets`-aware placement, not a contiguous-run search for a device) but freely overridable, per ADR 0001 — narrowed by ADR 0027 to the slot itself: you still choose where a device goes, you no longer choose its address independently of that. A Rack has no "purpose" field in the data model — a "spare rack" (e.g. a rack of spare amps) is just an ordinary Rack whose slots happen to hold spare equipment, and a rack created from a Rack Template is likewise just an ordinary Rack the moment it exists (see Rack Template). **An empty Rack is a reservation of its address block** — the way to hold offset space back from the first-fit suggester — and is likewise an ordinary Rack, with a `slot_count` that honestly states how many ordinals its block can address (ADR 0019; the boundary with ADR 0015's "`slot_count` stays honest" rule is drawn there). A Rack optionally carries `location_slug` (unique where set) — the hostname *location* component (ADR 0023), not a purpose field: this entry's "no purpose field" claim above still holds. `AVIO` and `SPARE` both carry a location slug despite being address pools rather than enclosures; only `CONSOLES` is blank — the field asks "does this rack have a location name?", not "is this rack virtual?".
_Avoid_: treating "spare rack" as a distinct type from Rack; treating a slot number as a physical rack-unit position; assuming a Rack is a physical enclosure; reaching for a separate "address pool" concept (ADR 0019); re-deriving "virtual racks are location-free" from `AVIO`/`CONSOLES`/`SHURE` above — `location_slug` is blank only on `CONSOLES`
...
**Type Profile**:
A Network Switch/Device Type is a *purpose profile* built on a hardware model, not the bare hardware model itself — the same physical hardware can have several profiles when what its ports are used for differs (e.g. a Cisco SG350-10MP wired for a drive rack vs. the same switch wired for an amp rack). `Name` is a required, non-blank profile label ("Default" for a model with only one profile). A Network Switch Type is identified by `(Manufacturer, Model, Name)`. A Network Device Type is identified by `(Device Model, Name)` instead — ADR 0026 gives the device side a real `Network Device Model` entity (manufacturer, model, and a description of what the hardware *is*) that profiles point at, rather than carrying their own manufacturer/model strings. The two Type models deliberately diverge until the switch side is aligned the same way (issue #78) — see Network Device Model.
_Avoid_: treating `(Manufacturer, Model)` as a Type's whole identity, or assuming one Type = one hardware model

**Network Device Model**:
The bare hardware identity a Network Device Type profile is built on (ADR 0026) — "an Amphenol RJD32A3-0050", independent of what any particular profile's ports are wired for. Carries `manufacturer`, `model`, a `description` of what the hardware *is* (e.g. "Dante Interface with AES3 I/O"), and (since ADR 0026 PR 2) `hostname_slug` — the description belongs here, not on the profile, because two profiles of one model describing the hardware differently is exactly the drift this entity exists to prevent, and the same is true of the slug: ADR 0023 makes it a **blocking** hostname component, so a divergent value used to silently compute different hostnames for identical hardware. Several Device Type profiles can point at one model (a Martin Audio IK-42 "with Dante Card" and "without Dante Card" are the same box). Unlike a Type profile, a model row is **not locked** after instances exist — editing its manufacturer/model/slug corrects every profile of that model coherently, which is the whole reason for the entity. Has no Network Switch Model equivalent yet (issue #78).
_Avoid_: assuming duplicate models (e.g. `Lab Gruppen LM26` alongside `Lab.Gruppen LM26`) are prevented — they are creatable, and merging them is unbuilt (issue #79); assuming the description appears in a Type's `__str__` — it doesn't, to keep operator-addressed error messages short and every existing row's rendering unchanged
...
A Network Device Type Port also carries a **Slot Offset** (default 0), copied onto its materialized Network Device Port — every static address in the system is `range_base + rack_slot + slot_offset` (ADR 0027), so this field is the whole addressing mechanism, not a special case bolted onto an otherwise-editable address. A non-zero offset covers two hardware shapes: firmware that derives one port's address from another's and refuses to let it be changed (e.g. a DiGiCo console's audio engine, always its control address + 1), and a second, independently addressed interface on a VLAN the device already uses (e.g. a Yamaha console's Device Control interface — ADR 0027 decision 3, closing #42 the same way, not through a separate addressing mode). A device claims the **set** of ordinals `{rack_slot + offset}` for each offset its type declares, not a contiguous range — a device with offsets `{0, 64}` claims exactly two ordinals and is indifferent to the 63 between them (ADR 0027 decision 2, closing #83). Every static address is derived and read-only, not independently editable, for every port alike — offset 0 included; setting the rack slot is the only way to set an address (ADR 0027 decision 1, superseding ADR 0003). See ADR 0017 for the offset mechanism's origin and ADR 0027 for its generalization.
_Avoid_: assuming an edit to a Type Port after instances exist affects those instances; assuming an instance's type can be changed instead of recreating it with a different Type; using Slot Offset for hardware whose parts are independently, manually addressed (that's two ordinary devices in two slots — see ADR 0017's scope-boundary section); assuming a device's occupied ordinals form a contiguous span from `rack_slot` — they don't, it's the declared-offset set (ADR 0027); assuming a stored address can ever disagree with what its ordinal computes — that state is unrepresentable now, not merely discouraged (ADR 0027 decision 5)

**Add-in Card**:
Optional, removable hardware — a DiGiCo DMI-DANTE, a Behringer X-Dante — bought separately from its host and taken in and out of it across its lifetime. Modeled as an ordinary Network Device (`Network Device Type.is_add_in_card`) with its own rack slot: **Rack** is this system's address pool, not a chassis, so a card holds an address in that pool whether or not anything currently hosts it, and a hostless card is legal. Carries a nullable `host` (self-referencing FK) recording which device it is *currently fitted to* — a soft pointer with **no addressing meaning whatsoever**: it does not derive an address, constrain a VLAN, share an ordinal, or move anything, and a pulled card keeps its rack, slot and addresses exactly as they were. See ADR 0022 tier 3, which sorts dependent hardware into three tiers by *optional* × *removable* rather than by addressing mechanism or procurement, and supersedes ADR 0018's Device Companion, which this entry replaces.
_Avoid_: assuming a card shares its host's rack slot or ordinal — it does not (ADR 0022 rejected that shape as machinery built only to reproduce a spreadsheet convenience); treating a built-in interface with no separate identity (a console's own Dante ports) as a card — see Network Device Type Port's Slot Offset for hardware that's a *port*, not a device; assuming `host` carries any of ADR 0018's old "companion" semantics (inseparable, materialized together, cascade-deleted) — it is the opposite: separable, fitted later, moves independently, and deleting a host clears the link rather than taking the card with it
