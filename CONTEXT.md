...
The bare hardware identity a Network Device Type profile is built on (ADR 0026) — "an Amphenol RJD32A3-0050", independent of what any particular profile's ports are wired for. Carries `manufacturer`, `model`, and a `description` of what the hardware *is* (e.g. "Dante Interface with AES3 I/O") — the description belongs here, not on the profile, because two profiles of one model describing the hardware differently is exactly the drift this entity exists to prevent. Several Device Type profiles can point at one model (a Martin Audio IK-42 "with Dante Card" and "without Dante Card" are the same box). Unlike a Type profile, a model row is **not locked** after instances exist — editing its manufacturer/model corrects every profile of that model coherently, which is the whole reason for the entity. Has no Network Switch Model equivalent yet (issue #78).
