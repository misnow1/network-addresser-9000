...
- ~~Slot moves don't re-suggest an already-static device port's address — see #28.~~ **Closed by ADR 0027**, which removes the staleness *class* rather than reporting it: a device's addresses are derived from its rack slot, so a move recomputes them (behind a confirmation) and a stored address can no longer disagree with its ordinal. The entry used to predict "one UI treatment should eventually serve both" #28 and #54 — that turned out to be wrong in an instructive way. #54's divergence is between a *name* and its components, which genuinely can only be reported; #28's was between an *address* and its ordinal, which could be made unrepresentable instead
...
- **The ordinal is the unit** (*closed by ADR 0027*). ADR 0019 made the rack the address pool; ADR 0003 kept device addresses stored and editable. Both could not be true, and the gap between them — an address that disagrees with its ordinal — was reachable, which is what generated #84, #83, #62, #28 and #60's whole feature. ADR 0027 derives every static address from `range_base + rack_slot + slot_offset`, retires `address_source`/`OPERATOR` (2 users in the estate, both resolvable to `+1`), and supersedes ADR 0003. The estate was already keeping the rule: **130 of 132** static racked device ports conformed before a line was written. Not a numbered phase, and deliberately **not** phase 20 — see ADR 0027 decision 7. `docs/plans/PLAN-adr-0027.md` builds it; **#93** carries the deferred tether encoding
