...
## Review response 2 (Codex round 2, post-implementation code review)

Independent review of the committed branch (`REVIEW-2-PLAN-adr-0018.md`), eight findings, no P0.
All eight fixed with tests — none contradicted committed ADR 0018 or the six decisions Mike
settled during grilling, so none was escalated.

| Note | Resolution | Section |
|---|---|---|
| 1 — P1 the companion's final placement is never validated (occupancy vs. a switch/other device, or `rack.slot_count`) | **Fixed.** `_finish_companion_move()` now calls `companion.full_clean()` (after setting `_host_managed_move`, so the placement lock it exists to bypass isn't tripped) before `save()`, reusing the same occupancy/bounds machinery `_materialize_companion()` already runs via `full_clean()` on creation, rather than hand-rolling a third copy. Covered by two new tests: overlap with an unrelated device, and exceeding a small rack's `slot_count`. | `_finish_companion_move()` |
| 2 — P1 partial `update_fields` moves read dirty in-memory state for the field *not* named | **Fixed.** `_plan_companion_move()` now derives each of `new_rack_id`/`new_rack_slot` independently — from `self` only when that specific field name is in the normalized `update_fields`, from the persisted value otherwise — rather than always reading both off `self`. Covered by a test that mutates both fields then calls `save(update_fields=["rack"])`. | `_plan_companion_move()` |
| 3 — P1 `NetworkDeviceTypeForm.__init__` 500s on any locked type's change page | **Fixed.** Confirmed real by hand (`KeyError: 'companion_type'`, reproduced against the live admin). `__init__` now returns early when `"companion_type" not in self.fields` — Django's admin drops readonly fields from the generated form entirely, so there's nothing to restrict on a form that doesn't carry the field. Covered by a form-level test (`modelform_factory` reproducing the admin's own field exclusion) and a full HTTP test opening a locked type's change page. | `NetworkDeviceTypeForm` |
| 4 — P2 a type's own current companion choice is excluded from its own dropdown | **Fixed.** Confirmed real by hand (200 with "Select a valid choice", reproduced against the live admin). The queryset now folds the instance's current `companion_type_id` back in via `Q(...) \| Q(pk=self.instance.companion_type_id)`, mirroring the `exclude(pk=self.instance.pk)` self-exclusion right below it. The *existing* `test_type_admin_companion_type_dropdown_excludes_chains_and_self` had been asserting the bug (excluding `self.host_type`'s own current companion) under a "already someone's companion" comment that conflated "someone" with "this same instance" — rewritten to exercise a genuinely different type's claimed companion instead, and a new test proves the current value now validates on resubmission. | `NetworkDeviceTypeForm` |
| 5 — P2 delete-selected refuses a selection containing both halves of a pair, or "select all" | **Fixed.** `NetworkDeviceQuerySet.delete()` now excludes a hosted row from the refusal when its host is also in the same selection (a stable up-front `pks` snapshot, not a live subquery against a queryset mid-delete). Covered by three tests: both halves selected, "select all", and the original companion-alone-refused case re-asserted unweakened. | `NetworkDeviceQuerySet.delete()` |
| 6 — P2 the verifier only checks `companion.host_id == host.pk`, missing a reverse-cycle corruption | **Fixed.** Added a direction assertion — `host.host_id is not None` fails independently of the existing pairing check, since `host` is a nullable OneToOne and nothing at the schema level stops two rows pointing at each other. Covered by a new test in `test_prod_import.py`, same `QuerySet.update()` bypass shape as the existing broken-link test. | `verify_prod_import.py` |
| 7 — P2 the linking migration is never actually exercised against 0010-shaped state | **Fixed, with a correction.** The core claim is correct and serious — grep confirms zero prior references to `0011` or `link_production_companions` anywhere in the suite. The finding's framing that rev 2 "committed to" `MigrationExecutor`/historical state is **not accurate** — rev 2's own review-response table (note 10) explicitly rejected that prescription in favor of the `real_apps` idiom `SeedDefaultsTests` uses for 0006. That idiom was tried first here and **fails**: verified by hand that calling `link_production_companions(real_apps, None)` against a live-migrated test database raises `ValidationError: companion_type cannot be changed after creation`, because `real_apps.get_model(...)` returns the *live* model class with its own lock enforcement, not a bare historical one — a difference that doesn't bite 0006's `seed_defaults()` because that function never writes to an ADR-0010-locked field. Landed on a third option, lighter than `MigrationExecutor`: `MigrationLoader(connection).project_state(("inventory", "0011_device_companions")).apps` — reconstructs field-shape-accurate model classes from the migration files with no custom `save()`, without ever stepping real schema DDL (the schema is already at HEAD for the whole suite), which is all a real `RunPython` step ever gets. Six new tests: case-insensitive success (the `dm7c-1`/`DM7C-1` case decision 4 exists for), zero matches, multiple matches, a host type already linked to a different companion type, idempotency on re-run, and an empty database. | New `DeviceCompanionMigrationTests`; migration's own docstring untouched |
| 8 — P2 the address-validation test can't fail — it only moves within one rack's range, where addresses always stay valid | **Fixed.** Added two tests that construct a move which actually invalidates a stale address: one cross-rack move where the *host's* own address no longer fits (different rack, different `RackVlanRange` subnet), and one where a third rack shares the host's own VLAN range verbatim (so the host's address still fits) but carries no range at all for the companion's VLAN, isolating a companion-only failure. Both assert the whole transaction rolled back — nothing moved on either row. The original test is left in place; it still proves the move path lives in `save()`, not `clean()`, which is a different, still-valid claim. | New tests alongside `test_bare_save_moves_companion_and_validates_both_addresses` |

## Review response 3 (Codex round 2, final review — plan-cycle caps at two)

Three new P1s, all in the model save path, all confirmed by hand before fixing (Mike independently
verified all three against the code before sending). All three fixed with tests; none rebutted,
none contradicted committed ADR 0018 or the six decisions Mike settled during grilling — note 1
touches "the companion's slot is its own occupancy, not part of the host's `slot_span`" only in
the sense of detecting overlap between two independent ranges, exactly what
`_check_companion_move_possible()` already exists to do.

| Note | Resolution | Section |
|---|---|---|
| 1 — P1 pair overlap escapes a bare `save()` when either type spans more than one ordinal | **Fixed.** Reproduced by hand exactly as described: a host spanning 5–6 (a non-zero `slot_offset` type, ADR 0017) committed a companion at 6 through a bare `save()`, no error. Root cause confirmed — `_check_companion_move_possible()` only runs from `clean()`; the companion's own `full_clean()` in `_finish_companion_move()` structurally excludes its host from occupancy conflicts (`_companion_pair_pks()`), since pair-vs-pair overlap is deliberately *not* that check's job; and `unique_device_rack_slot` compares starting slots only. Extracted the existing overlap arithmetic into `_check_pending_move_no_overlap()`, called both from `_check_companion_move_possible()` (the `clean()`-path pre-flight, unchanged behaviour) and now unconditionally from `save()`'s not-new branch, right after `_plan_companion_move()`, so a bare `save()` gets the same check a `full_clean()`-then-`save()` caller already did. | `save()`; `_check_pending_move_no_overlap()` (new, factored out of `_check_companion_move_possible()`) |
| 2 — P1 `_plan_companion_move()` reads a stale cached companion across a second move on the same host instance | **Fixed.** Reproduced by hand exactly as described: blank-preserving moves 1/2 → 6/7, then host → 8 on the same instance with no `refresh_from_db()` anywhere, computed companion slot 4, not 9 — confirmed **not** the round-1 relation-caching decision (that one was about a caller reading `host.companion` *after* a move without refreshing; this is the move-planning code's own bookkeeping computing a wrong target from state it manages itself, with no unusual caller behaviour involved). Root cause: `companion.rack_id`/`companion.rack_slot` were read off whatever `_get_related()`'s reverse-relation cache held, which `_finish_companion_move()` never updates — it writes through a *separately fetched* instance. Fixed by reading the companion's old rack/slot from a fresh `.values()` query by pk, the same pattern `old_rack_id`/`old_rack_slot` (the host's own) already used. | `_plan_companion_move()` |
| 3 — P1 host address validation in `_finish_companion_move()` reads dirty in-memory fields, not effective post-save values | **Fixed.** Reproduced by hand exactly as described: `save(update_fields=["rack"])` with in-memory `rack_slot=None` (excluded from `update_fields`, never persisted) persisted the new rack while `_validate_existing_addresses_still_fit()` — called directly on `self` — saw `rack_slot=None` and *silently skipped* its rack-range containment check (not a failure, `_address_containment_error()` just returns early whenever `rack_slot` is `None`), letting a stale, now-out-of-range host address survive. Confirmed the round-2 fix to `_plan_companion_move()` didn't cover this call site, which reads `self` directly rather than the move plan. Fixed by re-fetching the host's own row fresh (`NetworkDevice._default_manager.get(pk=self.pk)`) before validating — by that point `super().save()` has already written whatever `update_fields` actually persisted, so this reads the same effective values the move plan uses, without needing to change `_validate_existing_addresses_still_fit()`'s general signature (shared with `NetworkSwitch`). | `_finish_companion_move()` |
| Incidental — mypy flagged `new_rack_slot + (...)` as `int \| None + int` once finding 2's fix changed how `companion_old_rack_slot` is typed | **Fixed, not requested by the review.** Tracing it down turned up a real, if narrow, latent gap predating this round: a bare `save()` with `self.rack` set and `self.rack_slot` left `None` (an inconsistent in-memory state) reaches `_plan_companion_move()`'s arithmetic *before* `super().save()` ever gives the DB's `networkdevice_rack_and_slot_together` `CheckConstraint` a chance to reject it, so it would previously have crashed with a raw `TypeError` instead of a clean `ValidationError`. Added an explicit guard raising `ValidationError` for that state, plus a regression test — a bare `save()` reaches it directly, no `update_fields` trickery needed. | `_plan_companion_move()` |

## Review response 4 (Codex round 3 — extra round, past the plan-cycle cap)

Mike asked for a third code-review round beyond `/plan-cycle`'s two-round cap, on the grounds
that rounds 1 and 2 had *both* found P1s in the companion move path and that convergence was
weak evidence the subsystem was exhausted. It was not: five more findings, four of them P1,
all in that same path. The prior round's three fixes were confirmed correct and were not
implicated — these are states those fixes never reached. All five fixed with tests; none
rebutted; none contradicted committed ADR 0018 or the six decisions Mike settled 2026-08-02.

| Note | Resolution | Section |
|---|---|---|
| 1 — P1 overlap span derived from a dirty in-memory `device_type` | **Fixed.** `_check_pending_move_no_overlap()` computed `host_end` from `self.slot_span`, which reads through `self.device_type`. That field is locked, but `_check_locked_fields_unchanged()` only inspects fields named in `update_fields`, so `save(update_fields=["rack_slot"])` never looks at it — a persisted span-2 host holding an in-memory, never-to-be-persisted span-1 type validated ranges 5 and 6 as disjoint while the database kept the span-2 type and committed overlapping 5–6 and 6. Fixed by fetching the host's own row fresh and taking `slot_span` from it, the same pattern the `companion` on the line below already used. | `_check_pending_move_no_overlap()` |
| 2 — P1 an explicit companion-only relocation is silently discarded | **Fixed.** The early `if new_rack_id == old_rack_id and new_rack_slot == old_rack_slot: return None` fired before `_companion_rack_slot` was ever consulted, so submitting the host change form with the host's slot unchanged at 5 and the companion's edited from 4 to 3 reported success and moved nothing. This **implements** decision 1 ("a filled value is an explicit absolute slot") rather than reopening it. Fixed by moving the "nothing to move" test to the end of the function, after both rows' targets are computed, and requiring *both* the host and the companion to be unchanged before returning `None`. The companion's persisted position is now fetched before that decision rather than after. | `_plan_companion_move()` |
| 3 — P1 the consumed `_companion_rack_slot` is never cleared after a move | **Fixed.** `_materialize_companion()` already reset it after creation; `_finish_companion_move()` did not, and that asymmetry was the bug. Moving 5/4 → 8/10 with an explicit target, then reusing the same host instance for a blank-preserving move to 12, kept treating the stale 10 as explicit and left the companion there instead of preserving the new +2 offset at 14. Fixed by resetting it once the move it drove has actually happened. | `_finish_companion_move()` |
| 4 — P1 a concurrently deleted host is resurrected as an orphan | **Fixed.** A missing persisted row was treated as "nothing to move" and returned `None`. Every caller has already established `self.pk` is set and the row is not new, so a missing row means something else deleted this host mid-`save()` — visible mid-transaction under this project's READ COMMITTED isolation. Returning `None` let `super().save()` reach Django's "`UPDATE` affected 0 rows, so `INSERT` instead" fallback, and because `is_new` was captured as `False` at the top of `save()`, neither `_materialize_ports()` nor `_materialize_companion()` would run — resurrecting a host row with no ports and no companion, violating the ADR's core invariant. Now raises `ValidationError` telling the operator to reload. | `_plan_companion_move()` |
| 5 — P2 lock-order inversion deadlocks two concurrent moves of the same pair | **Fixed.** A colliding move parks the companion first (UPDATE companion, then UPDATE host); a non-colliding move writes the host first (UPDATE host, then UPDATE companion in `_finish_companion_move()`). Run concurrently, each holds one row and waits on the other. Fixed by acquiring both pair rows sorted by pk through `_lock_type_rows()` — which already solves the identical problem for type rows — before either row's own UPDATE. Locked only when a move is actually planned, so a hostname-only edit doesn't widen its lock window over a row it will never write. Two tests, one per ordering. | `save()` |

## Review response 5 (Codex round 3, requested beyond the plan-cycle's two-round cap)

Round 4's fixes were confirmed correct by Mike independently (committed as `d07c134`, 476 tests
green) — nothing in this round's four findings implicates them. Two of the four are *incomplete*
versions of round 4's own fixes: the same defect class, patched at one call site while a second
(finding 3) or a different exit path (finding 4) at the same defect went unpatched. All four
fixed with tests; none rebutted; none contradicted committed ADR 0018 or the six decisions Mike
settled 2026-08-02. Two (findings 2 and 3) are genuine cross-connection races — the deterministic
half of each is tested, and what remains unproven is stated plainly rather than papered over.

| Note | Resolution | Section |
|---|---|---|
| 1 — P1 in-memory state survives a rolled-back creation | **Fixed.** A new class, not a prior fix's gap. If `_materialize_companion()` raises after `super().save()` has inserted the host, the transaction rolls back the database row, but Django has already set `self.pk` and cleared `self._state.adding` on the in-memory object — a rollback doesn't undo Python attributes. Retrying the *same* instance after fixing whatever was wrong then computes `is_new = False`, takes the update path, its `UPDATE` affects zero rows (the row was rolled back), Django falls back to `INSERT`, and neither materializer runs — a host with no ports and no companion, the same end state as round 4 finding 4, reached by a different route. Fixed by wrapping `super().save()` through the materializers in `try`/`except` and, on the creation path only, restoring `self.pk = None` and `self._state.adding = True` before re-raising, so a retry takes the creation path again in full. | `save()` |
| 2 — P1 stale type object used after locking its row | **Fixed, deterministic half only.** A new class. `_lock_type_rows(NetworkDeviceType, self.device_type_id)` locks the row but doesn't refresh `self.device_type` — if another transaction changed unlocked type A's `companion_type` from B to C and committed first, this save acquires A's lock and then continues using the stale cached object: the compatibility check passes against B and `_materialize_companion()` materializes a B, while the database's now-locked A says C. Fixed by re-reading `self.device_type` from the database immediately after the lock, so both the compatibility check and materialization see what was just locked rather than what this instance loaded earlier. What's tested is the deterministic consequence — forcing the exact stale-cache state a real race would produce via a separately-fetched concurrent edit, not two real transactions — not the race's timing itself, which this single-connection suite can't produce. | `save()` |
| 3 — P1 the companion's vanished-row branch still returns `None` | **Fixed, deterministic half only.** Round 4 finding 4's fix, applied to the host's own persisted-row lookup but not the companion's — `companion_persisted is None` still returned `None`, reaching the same zero-row-`UPDATE`→`INSERT` fallback. Noted as the *more* likely branch to fire, since `host` is `CASCADE` and a concurrent host deletion removes the companion too — which branch fires is a race between two reads a few lines apart. Fixed by raising here too, matching the host's own message shape, rather than relocating the lock ahead of `_plan_companion_move()`'s reads (that method also runs unlocked from `clean()`'s pre-flight, so locking ahead of it isn't free, and doesn't change that these are still two separate unlocked reads a few lines apart — raising closes the silent-resurrection *outcome*, not the underlying read-read race window itself, which remains as stated). Deterministic test detaches the companion (documented `QuerySet.update()` bypass) while leaving the host row genuinely present, forcing exactly this branch; the two-real-transaction race itself is unproven. | `_plan_companion_move()` |
| 4 — P1 the no-op return path doesn't clear the consumed slot | **Fixed.** Round 4 finding 3's fix lived inside `_finish_companion_move()`, which never runs when `host_unchanged and companion_unchanged` are both true (an explicit `companion_rack_slot` equal to the companion's current slot, with the host stationary) — a real exit path that consumes the input without ever reaching the method that reset it. Per the review's own suggestion, moved the reset to `save()` itself, immediately after `_plan_companion_move()` returns, regardless of what it returned — the single site covering every exit from a save that consulted the input, not a second per-branch patch. The now-redundant reset inside `_finish_companion_move()` was removed. | `save()`; `_finish_companion_move()` |
| Audit — round 3/4 fixes checked for other unpatched sites of the same class | **One more found and fixed, unprompted.** Every remaining `self.<cached-relation>` read in the four move-path helpers (`_plan_companion_move()`, `_park_companion_if_colliding()`, `_finish_companion_move()`, `_check_pending_move_no_overlap()`) was traced by hand. `_park_companion_if_colliding()` (`:4008`) used `self.slot_span` for the same reason round 4 finding 1 did — a third instance, not a second. Verified empirically (not just argued) that a dirty span here can't produce a wrong *final* placement, since `unique_device_rack_slot` only enforces exact `(rack, rack_slot)` equality, which doesn't depend on either row's span — but it *does* change whether a park that should have happened actually does, producing a single direct-move audit entry instead of a truthful parked pair (round 2 finding 6's concern). Fixed for consistency and to remove the audit-trail inaccuracy, with a test asserting the two-entry parked shape. `NetworkDeviceType.save()`'s own analogous lock-then-use pattern (`:2648`) was checked and found already correct — `_validate_companion_type()`'s own docstring states it reads every value fresh from the database rather than trusting a cached relation, which tracing confirmed. `NetworkDevice.delete()`'s companion guard was checked and found already correct — it reads `_persisted_host_id()` fresh rather than `self.host_id`. `NetworkSwitchTypePort.save()` has a structurally similar lock-then-use shape but is pre-existing code untouched by any ADR 0018 round; noted, not audited further — out of this review's scope. | `_park_companion_if_colliding()` |
